...
# Đọc dữ liệu
positive_df = pd.read_excel('../data_positive_full.xlsx')
negative_df = pd.read_excel('../data_negetive_full.xlsx')
...
merged_df['Reviews'] = merged_df['Reviews'].apply(
    lambda x: ' '.join([word for word in x.split() if word not in stop_words]))
merged_df['Reviews'] = merged_df['Reviews'].apply(word_tokenize)

# Chia dữ liệu tập huấn
X_train, X_test, y_train, y_test = train_test_split(merged_df['Reviews'], merged_df['Sentiment'], test_size=0.2,
                                                    random_state=42)
